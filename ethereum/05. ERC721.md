...
contract MusicNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    // 採掘されたNFTの tokenID などの情報を通知するイベント
    event Mint(address to, string uri, uint256 tokenId);
    ...
    // このコントラクトをデプロイした所有者のみが新たなNFTを採掘できる
    function safeMint(address to, string memory uri) public onlyOwner {
        uint256 tokenId = _nextTokenId++;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
        // 採掘されたNFTの情報を通知するイベントの発出
...
describe("MusicNFTコントラクト", function () {
  ...
  it("音楽家 addr1 の音源 music1 の MusicNFTの生成", async function () {
    ...
    // 最初のNFTのTokenID は 0 
    console.log(await MusicNFT.ownerOf(0));
    // TokenURIの確認
    console.log(await MusicNFT.tokenURI(0));
...
https://music.example/https://music.example/music1.json
    ✔ 音楽家 addr1 の音源 music1 の MusicNFTの生成 (397ms)
    ✔ NFTの所有の移転
