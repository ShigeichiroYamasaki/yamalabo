...
最終更新 2024/11/02  Shigeichiro Yamasaki
...
// JPYcoin (ERC20)コントラクトを利用するためのインターフェース定義
interface JPYcoin {
...
// トークン販売コントラクト
contract Faucet {
    JPYcoin public token;  // トークンコントラクト
...
│   └── contracts
├── build-info
│   ├── 2ae430f2ba62aa8c55e3286f30a23bfa.json
│   └── f11e0b5074104693541d5d2ffe9da1b7.json
└── contracts
    ├── JPQ.sol
    └── TokenSale.sol
...
async function main() {
  ...
  console.log("デプロイ主体のアドレス:", deployer.address);

  // トークン名，トークンの総供給量を初期指定して
  // JPQコントラクトのABIやバイトコードを持つトランザクションを構成し
  // トランザクションをブロードキャストしてデプロイする
  const JPYcoin = await hre.ethers.deployContract("JPYcoin",[10**11]);
  // デプロイの完了を待つ
  await JPYcoin.waitForDeployment();
  // トークン販売コントラクトをデプロイする
    //    (1ETH = 408000円 = 408000JPQ)
  const TokenSale = await hre.ethers.deployContract("TokenSale",[JPYcoin, deployer.address, 408000]);
  await TokenSale.waitForDeployment();
  console.log("TokenSaleコントラクトアドレス:", TokenSale.address);
}
