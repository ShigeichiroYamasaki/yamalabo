...
* Provider :  ethereum ネットワークに接続してブロックチェーンの状態やイベントログを参照（読み取り専用）
...
* Receipt: トランザクションがブロックに格納されるとレシートが利用可能になる．トランザクションが格納されたブロックや消費したガスや発生したイベントなどの情報を得ることができる
