...
# 数値にエンコード
def encode(m):
    code = base64.b64encode(m.encode())
    return (int.from_bytes(code,'little'),len(code))

>>> num,l =encode(m)
>>> num
3235199842280327765224261484107541982483167718442127528151054142879834361564138973780045833349466209858612
>>> l
44
...
>>> key = genkey(num)
>>> key
857448620764902181144852004166265761725484473188686380327057048957583363966721733655412153354448233643925
