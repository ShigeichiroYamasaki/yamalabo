...
2. CO2（炭酸水素ナトリウム＋クエン酸）
   1. ファンを使って４秒 ~ 6秒間隔でCO2濃度を変化させる（動物の呼吸を模倣）
   2. ファンでは，CO２と併せて匂いも拡散させる 
3. 体温（36°C）
4. 色彩（黒，青，赤と明暗コントラスト）
5. 皮膚模造パッド（色が重要）
...
最大の特徴は、「蚊そのもの」を識別しているのではなく、「蚊らしい振る舞い」を識別していること。

これは、現在の研究で考えられているトンボの小型移動物体検出（STMD）と発想が近く、以下の条件を組み合わせて判定。
...
画像センサーを使わなくても、18個程度（将来的にはその倍）のフォトトランジスタから得られる限られた情報を時間方向も利用して解析することで、単純な遮光センサーよりはるかに高い認識性能が期待できます。
...
時期バージョンでは，左右2個の赤外線LEDによる視差（パララックス）を利用した立体的な物体認識による誤検知の減少処理を追加する
...
* 影のパタンの急激な変化で動体を検知する
* （次期バージョン）両眼の視差を利用して誤検知を減らす
* 蚊の通過は 数ms ~ 数百ms: 動きがあるなら蚊と判断する
...
発光側を２灯の赤外線LEDを時差で発光させることで，トンボの両眼による視差を再現する
...
赤外線LED はレンズで中央部への集光を行っているので周辺部の光量が少ない

受光側のフォトトランジスタは，場所による受光量の違いを初期キャリブレーションで調整して均一化する必要がある．
...
![](https://github.com/Freenove/Freenove_ESP32_S3_WROOM_Board_Lite/blob/main/Board.png?raw=true)

このシステムのために現状で最適なボードはこれだと思います．

* 18個（将来的にはその倍の数）のフォトトランジスタの入力をリアルタイムで観測できる
* 蚊の捕獲ログを記録できるストレージを持つ
* web サーバとして捕獲記録を外部から参照できる
...
★（注意！）：基盤に印刷されている番号と GPIO番号は異なっているので，この図を確認しながら接続すること！！！
...
吸引力は 交流モーターのものが圧倒的に有利だが，コンパクト性とUSBケーブルだけでよいという配線の単純さを考えてDCモーターのものを利用することにしたが，実際には交流電源ケーブルを利用するので将来的には交流モータのハンディ掃除機を利用する．

将来的にはAC掃除機に変えるかもしれない．

![](https://m.media-amazon.com/images/I/61AI7mS2TOL._AC_SL1500_.jpg)
...
CD74HC4067×2、フォトトランジスタ18個、IR LED2個、サーボ1個の配線確認用テストプログラム
...
void pressOnce(int pressAngle, int restAngle)
{
...
void setup()
{
    ...
    Serial.println("0-180   : move");
    Serial.println("+       : +5 deg");
    Serial.println("-       : -5 deg");
    Serial.println(">       : +10 deg");
    Serial.println("<       : -10 deg");

    Serial.println("r       : center 90");
    Serial.println("p       : press test");

    Serial.println("a       : auto sweep");
