...
* 天敵であるトンボの複眼の模倣
* 藪蚊の視覚で赤外線は見えないので赤外線を利用
* 複数の受光センサーにより，蚊の影を検出する
...
* 発光側：940nm赤外線LEDを 2KHz 程度で点滅させる
* 受光側：フォトトランジスターからの信号をフーリエ変換して 2KHz 以外の周波数の出現で蚊を認識する
* 発光側と受光側の光軸を平行に 1cm間隔で6本並べて，蚊がその光軸を横切ったことを認識する
...
#### 問題
1mm 程度の光軸で発光側と受光側を一致させる必要があるので，高度な工作精度が求められる．
...
* 発光側：６個の940nm赤外線LEDを時間差で一つずつ発光させる
* 受光側：多数のフェトトランジスタで，受光値の変化を調べ，変化したことで蚊の存在を知る
