...
brew install llvm autoconf gcc make curl direnv sqlite3 sqlite-utils libsigsegv leveldb zlib zlib-ng git nodejs node npm m4 wget curl rbenv libyaml rust
```
...
sudo apt update
sudo apt upgrade -y
sudo apt install -y curl git wget autoconf gcc make libssl-dev libffi-dev libz-dev  libyaml-dev sqlite3 sqlite-utils rustc
