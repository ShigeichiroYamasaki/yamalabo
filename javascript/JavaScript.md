...
オブジェクトが（継承を含めて）そのプロパティを持っているかテストできる
...
// HTTP リクエストに対するレスポンスイベントを受け取ったときのコールバック関数の登録
...
2. resp オブジェクトの text() メソッドの非同期処理によって，HTML の body 部を取り出します．それが完了すると 変数 body に結果を代入します．
...
上記のプログラムで await をつけないと，以下のように 変数 resp2 は fetch() による http GETの完了を待たない状態のオブジェクトが代入されるので，その内容を得ることができません．

```js
> let url2 = 'https://www.kindai.ac.jp/'
> let resp2 = fetch(url2)
> let body2 = resp2.text()
Uncaught TypeError: resp2.text is not a function
```
...
Node.js では，一つの Node.js アプリケーションシステムを「パッケージ」と呼んでいる．
Node.js のパッケージは，他のアプリケーションフレームワークにおける「プロジェクト」と考えてよい．
...
#### ファイル拡張子を .mjs にしたモジュールのエクスポートとインポート
