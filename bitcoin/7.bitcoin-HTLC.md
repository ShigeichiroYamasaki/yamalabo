...
AliceとBobは異なるワレットやノードで操作するとよいでしょう
...
Alice に 0.001 btc 送金する

```bash
bitcoin-core.cli send '{"tb1q92v4dxsz47zxs5rdu42q7nl4xsdlncmvswxr5f": 0.001}'
...
### AliceはCarolから秘密情報 `<Secret>` の OP_SHA256 ハッシュ値を得る
...
output (デポジット金額：0.0001,手数料 0.00002)
...
## Alice によるHTLCロックトランザクション作成処理のまとめ
...
```ruby
secret='HTLC_test'
secret_hash_Carol=Bitcoin.sha256(secret)
```

Bobの処理

```ruby
pubkeyBob_Bob = keyBob.pubkey
```

* Aliceは、Carolが生成したsecret_hashを得る
* Aliceは、Bobの公開鍵を得る

Aliceの処理をメソッドにまとめる

```ruby
# Carolが生成した secret_hash (Carolからもらう)
secret_hash = secret_hash_Carol
# Bobの公開鍵（Bobからもらう）
pubkeyBob = pubkeyBob_Bob
# HTLCでデポジットする金額
deposit =0.0001
# 手数料
fee=0.00002
...
def create_HTCL_lock_transaction(keyAlice, secret_hash, pubkeyBob, deposit, fee, lockDays)
    ...
    unless (deposit > balance) or (utxos == []) then
        ...
        puts "you don't have enough deposit funds"
    end
end
```
...
```ruby
# HTLC ロックトランザクションの生成
htcl_lockTx  = create_HTCL_lock_transaction(keyAlice, secret_hash, pubkeyBob, 0.0001, 0.00002, 10)

=> 
[#<Bitcoin::Tx:0x00007f1a50ec2298
...
irb(main):320:0> htcl_lockTx
=> 
#<Bitcoin::Tx:0x00007f1a50ec2298 
 @inputs=
  [#<Bitcoin::TxIn:0x00007f1a50ec20e0
    @out_point=#<Bitcoin::OutPoint:0x00007f1a50ec2130 @index=1, @tx_hash="ac992c635a54898e8b8e40eaf63229c112db5605609cbf19ac332d77b3995ed2">,
    @script_sig=#<Bitcoin::Script:0x00007f1a50ec20b8 @chunks=[]>,
    @script_witness=
     #<Bitcoin::ScriptWitness:0x00007f1a50ec2068
      @stack=
       ["0D\x02 +\xDBB\xD5\xAD\x96\xC1\xCE\xFC\xBE\x0Fa\xA4\xCDq}?\x1A\xAB\xA8\f\rx\xE4\xA6\xB7{\xAA\xD2\xED#\xDC\x02 B/\xFC\xB1\x9A\x03>G\xB9\xB9\xAD\xF8\xE9\xCD\xE9\xD4\xEF4\xD7\x9B\xB8[\xDD\xAFH\v9q\xD9\xFC\xCD\x15\x01",
        "\x02\xF5\x1A\xEA\x05\x86$\x8F\x95(\xB9m\x13\xFD\x15]\x06\xC3\x94\xFBm\xC5\xD7\x90V\x857\xBEh\xC7^\xAF\xF7"]>,
    @sequence=4294967295>],
 @lock_time=0,
 @outputs=
  [#<Bitcoin::TxOut:0x00007f1a50ec6a78
    @script_pubkey=
     #<Bitcoin::Script:0x00007f1a50ec6ca8 @chunks=["\x00", " \x1F`qq\xE9\xD9\x90V\xCA\fWJ\xE2X\xCCH\xE3\x97\x06a0\xD8DuM)T\xAF\xA6`=\x9C"]>,
    @value=10000>,
   #<Bitcoin::TxOut:0x00007f1a50ecbc80
    @script_pubkey=#<Bitcoin::Script:0x00007f1a50ecbfa0 @chunks=["\x00", "\x14*\x99V\x9A\x02\xAF\x84hPm\xE5T\x0FO\xF54\e\xF9\xE3l"]>,
    @value=75998>],
 @version=1>


# TLCロックトランザクションのブロードキャスト
htcl_lockTx_txid = bitcoinRPC('sendrawtransaction',[htcl_lockTx.to_hex])

# HTLCロックトランザクションのトランザクションID（Bobにわたす）
htcl_lockTx_txid 
=>
"4637e606ac804383cf15a72dcad26347372c52b5fa2a4284ecede429088c00a7"
