...
[シェルスクリプトの利用](./9.shellscript)を前提とします
...
## bitcoin testnet4 のCPUマイニング

bitcoin mainnet でのマイニングは CPU でのマイニングは現実的ではありませんが，testnet4 は難易度が低いので現実的に可能です．

以下では，自分のマシンに testnet4 のノードが稼働していることを前提にします．

また，設定ファイル `bitcoin.conf` の内容は，上記のものとします．

特に rpc user のユーザー名とパスワードを利用していますが，必要に応じて変更してください．
...
```bash
ADDR=`bitcoin-core.cli getnewaddress`
```

### CPUマイニングのツール cpuminer のインストール
...
```bash
./minerd -a sha256d  -o http://127.0.0.1:48332 -u hoge -p hoge --coinbase-addr=$ADDR
```

### マイニングが成功したことの確認方法

testnet4 でもそれなりの時間がかかります

* 自分に関連するトランザクションの一覧を出力する
  
```bash
bitcoin-core.cli listtransactions 
```

* マイニング関連のトランザクションを確認する

```bash
watch -n 3 'bitcoin-core.cli listtransactions "*" 5'
```

`category: "generate" ` の表記があればそれがマイニング報酬